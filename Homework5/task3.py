...
def everything_for_your_cat(cats_data):
    owners_dict = {}  # Создаем пустой словарь для хранения данных о владельцах и их кошках
    for cat in cats_data:  # Проходим по каждой записи о кошке и её владельце
        name, age, owner_name, owner_surname = cat  # Распаковываем кортеж в переменные
        owner = owner_name + " " + owner_surname  # Соединяем имя и фамилию владельца
        if owner in owners_dict:  # Если владелец уже присутствует в словаре
            owners_dict[owner].append((name, age))  # Добавляем запись о кошке в список его питомцев
        else:
            owners_dict[owner] = [(name, age)]  # Иначе, создаем новую запись о владельце и его питомцах
    our_str = ""  # Создаем пустую строку, в которую будем добавлять данные о владельцах и их кошках
    for owner, cats in owners_dict.items():  # Проходим по каждому владельцу и его питомцам в словаре
        cat_str = "; ".join([f"{name}, {age}" for name, age in cats])  # Создаем строку с данными о питомцах
        our_str += f"{owner}: {cat_str}\n"  # Добавляем строку с данными о владельце и его питомцах в общую строку
    return our_str  # Возвращаем сформированную строку с данными о владельцах и их кошках
