...
def repeats(our_str):
    repeats_dict = {}  # Создаем пустой словарь для хранения количества повторений символов
    new_str = ""  # Создаем пустую строку, в которую будем добавлять символы с количеством повторений
    for char in our_str:  # Проходим по каждому символу в исходной строке
        if char in repeats_dict:  # Если символ уже присутствует в словаре
            repeats_dict[char] += 1  # Увеличиваем счетчик повторений для данного символа на 1
        else:
            repeats_dict[char] = 1  # Иначе, добавляем символ в словарь и устанавливаем счетчик повторений на 1
        new_str += char + "_" + str(repeats_dict[char])  # Добавляем символ и его количество повторений в новую строку
    return new_str  # Возвращаем новую строку с добавленными количествами повторений
