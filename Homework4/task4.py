...
def multiplication_chain(num):
    count_multy = 0  # инициализируем количество итераций
    while num >= 10:  # пока число имеет более одной цифры
        product = 1  # инициализируем произведение цифр числа
        for digit in str(num):  # перебираем цифры числа
            product *= int(digit)  # перемножаем цифры
        num = product  # заменяем число на произведение цифр
        count_multy += 1  # увеличиваем количество итераций
    return count_multy
