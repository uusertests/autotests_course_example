...
# Здесь пишем код
# Определяем базовый класс PublicTransport
class PublicTransport:
    ...
    @property
    def info(self):
        # Возвращаем информацию о марке, цвете, годе выпуска и мощности двигателя
        return f"Brand: {self.brand}, Color: {self.color}, Year: {self.year}, Engine Power: {self._engine_power}"


# Определяем класс Bus, наследующийся от PublicTransport
class Bus(PublicTransport):
    def __init__(self, brand, engine_power, year, color, max_speed, passengers, park, fare):
        ...
        self.park = park  # Приватный атрибут, доступный только внутри класса
        self._fare = fare  # Защищенный атрибут, доступный внутри класса и его подклассов
    ...
    @park.setter
    def park(self, value):
        # При присвоении значения проверяем, что номер парка находится в диапазоне от 1000 до 9999
        assert 1000 <= value <= 9999, "Invalid park number. Park number should be in the range of 1000 to 9999."
        self.__park = value


# Определяем класс Tram, наследующийся от PublicTransport
class Tram(PublicTransport):
    def __init__(self, brand, engine_power, year, color, max_speed, route, path, fare):
        super().__init__(brand, engine_power, year, color, max_speed)
        self.__route = route  # Приватный атрибут, доступный только внутри класса
        self.path = path
        self._fare = fare  # Защищенный атрибут, доступный внутри класса и его подклассов

    @property
    def how_long(self):
        # Вычисляем время прохождения маршрута по формуле max_speed/(4*path)
