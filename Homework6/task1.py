# Глобальные перемены.
# Имеется следующие переменные, определенные в глобальной области видимости модуля:
# number = 1
# string = 'Hello'
# Напишите и вызовите функцию, которая будет изменять и возвращать эти переменные, на следующие значения:
# number = 5
# string = 'Hello, dear friend'
#

number = 1
string = 'Hello'


def global_changes():
    global number, string  # Используем ключевое слово global для указания на глобальные переменные number и string
    number = 5  # Изменяем значение глобальной переменной number на 5
    string = 'Hello, dear friend'  # Изменяем значение глобальной переменной string на 'Hello, dear friend'
    return number, string  # Возвращаем измененные значения глобальных переменных number и string


global_changes()

# Ниже НИЧЕГО НЕ НАДО ИЗМЕНЯТЬ
assert number == 5, 'Переменная number должна иметь значение 5'
assert string == 'Hello, dear friend', 'Переменная string должна иметь значение Hello, dear friend'
assert global_changes() == (5, 'Hello, dear friend')

print('Все ок')
